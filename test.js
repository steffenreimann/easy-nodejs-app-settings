...
async function ASYNC_AWAIT() {
	...
	var DataStore = new fm.File({
		appname: 'easy-nodejs-app-settings-test',
		file: 'test.json',
		interval: 5000,
		data: {
			name: 'herbert24'
	...
	try {
		...
		//fm.Files;
		console.log('fm.Files.test.data 1 ', fm.Files.test.data);
		...
		console.log('fm.Files.test.data 2 ', fm.Files.test.data);
		process.exit(0);
	} catch (err) {
		//deleteFolderRecursive(pathToDir);
